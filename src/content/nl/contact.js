// Contact page content.

export const contact = {
  meta: {
    title: 'Contact',
    description:
      'Een product of platform dat gebouwd moet worden? Vertel het de studio. Je krijgt binnen 24 uur antwoord.',
  },
  eyebrow: 'Neem contact op',
  titleLine1: 'Laten we',
  titleLine2: 'praten',
  titleAccent: '.',
  lead: 'Vertel ons wat je bouwt of wat je tegenhoudt.',
  form: {
    nameLabel: 'Naam',
    namePlaceholder: 'Je naam',
    emailLabel: 'E-mail',
    emailPlaceholder: 'jij@bedrijf.com',
    companyLabel: 'Bedrijf',
    companyOptional: 'optioneel',
    companyPlaceholder: 'Bedrijfs- of projectnaam',
    messageLabel: 'Bericht',
    messagePlaceholder:
      'Wat ga je bouwen? Wat is het probleem, voor wie is het, en waar sta je nu — idee, half af, of live en vastgelopen? Hoe concreter, hoe beter.',
    sendLabel: 'verstuur bericht',
  },
  directContact: {
    label: 'Of mail ons direct',
    email: 'hello@raoul.studio',
    phoneLabel: 'Of bel',
    phone: '+31 10 307 3755',
    phoneHref: 'tel:+31103073755',
  },
  faq: {
    label: 'FAQ',
    items: [
      {
        q: 'Hoe snel hoor ik iets terug?',
        a: 'Elke echte brief wordt gelezen, met een antwoord binnen 24 uur. De studio blijft klein en direct, dus wie antwoordt staat dicht bij het werk.',
      },
      {
        q: 'Wat moet ik in mijn bericht zetten?',
        a: 'Vertel ons wat je bouwt of wat je tegenhoudt: het probleem, voor wie het is, en waar je nu staat — idee, half af, of live en vastgelopen. Hoe concreter de brief, hoe beter het antwoord.',
      },
      {
        q: 'Waar zitten jullie?',
        a: 'De studio zit in Rotterdam en werkt met teams waar ze ook zitten. Locatie is geen belemmering — het werk is samenwerking en wordt in de openheid met je gebouwd.',
      },
      {
        q: 'Wat voor projecten nemen jullie aan?',
        a: 'Maatwerkproducten en -platforms voor fintech, advocatenkantoren, commerciële bureaus en e-commerce — webplatforms en SaaS, headless e-commerce, AI-tools en automatisering, CRMs en interne tooling, APIs en tweezijdige marktplaatsen.',
      },
      {
        q: 'Hoe bereik ik jullie direct?',
        a: 'Mail naar hello@raoul.studio, of gebruik het formulier op deze pagina. Beide komen direct bij ons terecht — geen intakelaag ertussen.',
      },
    ],
  },
  posterRailMiddle: 'Laten we praten · Neem contact op',
}
